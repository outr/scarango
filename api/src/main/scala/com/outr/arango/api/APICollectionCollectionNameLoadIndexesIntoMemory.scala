...
object APICollectionCollectionNameLoadIndexesIntoMemory {
  /**
  ...
  */
  def put(client: HttpClient, collectionName: String)(implicit ec: ExecutionContext): Future[Json] = client
    ...
    .path(path"/_api/collection/{collection-name}/loadIndexesIntoMemory".withArguments(Map("collection-name" -> collectionName)), append = true)
    .call[Json]
}