...
object Value {
  ...
  implicit def int(value: Option[Int]): Value = Value(value.map(num).getOrElse(Null))
  implicit def long(value: Long): Value = Value(num(value))
  implicit def long(value: Option[Long]): Value = Value(value.map(num).getOrElse(Null))
