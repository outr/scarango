...
class APICollectionCollectionNameLoad(client: HttpClient) {
  /**
  ...
  */
  def put(collectionName: String): Future[ArangoResponse] = client
    ...
    .path(path"/_db/_system/_api/collection/{collection-name}/load".withArguments(Map("collection-name" -> collectionName)))
    .call[ArangoResponse]
}