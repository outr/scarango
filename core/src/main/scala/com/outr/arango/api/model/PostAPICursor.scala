...
/**
  ...
  *        not set, a server-controlled default value will be used. A {@literal *}batchSize{@literal *} value of
  *        {@literal *}0{@literal *} is disallowed.
  * @param bindVars key/value pairs representing the bind parameters.
  * @param cache flag to determine whether the AQL query results cache
  *        shall be used. If set to {@literal *}false{@literal *}, then any query cache lookup will be skipped
  *        for the query. If set to {@literal *}true{@literal *}, it will lead to the query cache being checked
  *        for the query if the query cache mode is either {@literal *}on{@literal *} or {@literal *}demand{@literal *}.
  ...
  *        case it allocates too much memory. A value of {@literal *}0{@literal *} indicates that there is no
  *        memory limit.
