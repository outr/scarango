...
/**
  ...
  *        - {@literal *}bytes_accum{@literal *}: consolidate if and only if ({threshold} range `[0.0, 1.0]`):
  *          {threshold} > (segment_bytes + sum_of_merge_candidate_segment_bytes) / all_segment_bytes
  *          i.e. the sum of all candidate segment byte size is less than the total
  *               segment byte size multiplied by the {threshold}
  *        - {@literal *}tier{@literal *}: consolidate based on segment byte size and live document count
