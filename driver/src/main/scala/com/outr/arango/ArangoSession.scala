...
import io.youi.http.{HttpRequest, HttpResponse, Method, StringContent}
...
class ArangoSession(val instance: Arango, val token: Option[String]) {
  ...
  def parse(query: String): Future[ParseResult] = {
    ...
    restful[ParseRequest, ParseResult]("query", ParseRequest(query), errorHandler = Some((request, response) => {
