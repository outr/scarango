...
class ArangoSession(val instance: Arango, var token: Option[String], credentials: Option[Credentials]) {
  protected[arango] def restful[Request, Response](db: Option[String],
    ...
    val fallbackHandler = (httpRequest: HttpRequest, httpResponse: HttpResponse) => {
      if (httpResponse.status == Status.Unauthorized && retryAttempts > 0) {     // Attempt reconnect
        ...
        errorHandler.getOrElse(instance.defaultErrorHandler(request))(httpRequest, httpResponse)
      }
    }
  ...
  protected[arango] def call[Response](db: Option[String],
    ...
    val fallbackHandler = (httpRequest: HttpRequest, httpResponse: HttpResponse) => {
      if (httpResponse.status == Status.Unauthorized && retryAttempts > 0) {     // Attempt reconnect
        ...
        errorHandler.getOrElse(instance.defaultErrorHandler(path))(httpRequest, httpResponse)
      }
    }
  ...
  def parse(query: String): Future[ParseResult] = {
    ...
    restful[ParseRequest, ParseResult](None, "query", ParseRequest(query), errorHandler = Some((request, response) => {
